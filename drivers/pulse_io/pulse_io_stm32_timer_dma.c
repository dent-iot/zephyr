...
#define DT_DRV_COMPAT st_stm32_pulse_io
...
#define PULSE_IO_STM32_INIT(inst)                                                   \
	BUILD_ASSERT(DT_INST_NODE_HAS_PROP(inst, timer),                            \
		     "st,stm32-pulse-io requires a 'timer' phandle");               \
	BUILD_ASSERT(DT_INST_NODE_HAS_PROP(inst, dmas),                             \
		     "st,stm32-pulse-io requires a 'dmas' property");               \
	BUILD_ASSERT(DT_INST_NODE_HAS_PROP(inst, gpios),                            \
		     "st,stm32-pulse-io requires a 'gpios' property");              \
                                                                                    \
...
\
	static const struct pulse_io_stm32_config pulse_io_stm32_cfg_##inst = {     \
		...
		.set_bit = BIT(DT_GPIO_PIN(DT_DRV_INST(inst), gpios)),              \
